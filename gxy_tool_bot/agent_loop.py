...
def run_agent_loop(
    ...
    repeat_tool_key: str | None = None
    repeat_tool_count: int = 0
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        if response.tool_calls:
            ...
            for tc in response.tool_calls:
                ...
                # Track consecutive identical tool calls with no other tool calls in between
                tool_key = f"{tc.name}:{json.dumps(tc.arguments, sort_keys=True)}"
                if tool_key == repeat_tool_key:
                    repeat_tool_count += 1
                else:
                    repeat_tool_key = tool_key
                    repeat_tool_count = 1
                if repeat_tool_count >= _REPEAT_TOOL_THRESHOLD:
                    logger.warning("Agent has called %s with identical args %d times consecutively — injecting nudge", tc.name, repeat_tool_count)
                    messages.append({
                        "role": "user",
                        "content": (
                            f"You have called `{tc.name}` with the same arguments {repeat_tool_count} times in a row "
                            "without running any other tools. The result is not changing — "
                            "step back and reconsider your approach. Try different arguments, "
                            "use a different tool, or use `give_up` if you're stuck."
                        ),
                    })
                    repeat_tool_count = 0  # reset to avoid spamming the nudge

...
def _contributing_section(issue: int, tool_dir: str, repo: str | None = None) -> str:
    ...
    if repo:
        ...
        fork_cmd = "# gh repo fork <owner/repo> --clone\n# cd <repo-name>"
    return (
        "\n\n---\n\n"
        "## Working on this tool\n\n"
        "To review or improve this tool locally:\n\n"
        "**If you don't have the repo yet** (forks and clones in one step):\n\n"
        "```bash\n"
        f"{fork_cmd}\n"
        ...
        f"git checkout {branch}\n"
        "```\n\n"
        f"The tool files are in `tools/{tool_dir}/`.\n"
    )
                ...
                if pr_body_path:
                    if validation_errors:
                        ...
                        for err in validation_errors:
                            pr_body += f"- {err}\n"
                    pr_body += _contributing_section(issue, tool_dir, config.repo)
